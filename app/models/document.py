...
class Document(Base):
    ...
    ingest_event = relationship(
        "IngestEvent", back_populates="documents", foreign_keys=[ingest_event_id]
    )
    embeddings = relationship(
        "DocumentEmbedding", back_populates="document", cascade="all, delete-orphan"
    )
    sentiments = relationship(
        "DocumentSentiment", back_populates="document", cascade="all, delete-orphan"
    )
    cluster_memberships = relationship(
        "ClusterMember", back_populates="document", cascade="all, delete-orphan"
    )
    umap_projections = relationship(
        "UMAPProjection", back_populates="document", cascade="all, delete-orphan"
    )
