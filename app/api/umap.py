...
from app.services.umap_service import get_cluster_umap_projections, get_user_umap_projections
...
@router.get("/clusters/{cluster_id}", status_code=status.HTTP_200_OK, response_model=ClusterUMAPResponse)
...
@router.get("/documents", status_code=status.HTTP_200_OK, response_model=ClusterUMAPResponse)
async def get_user_umap_endpoint(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    model_name: str = Query(DEFAULT_MODEL_NAME, description="UMAP model name"),
    limit: int = Query(1000, description="Maximum number of projections to return", ge=1, le=5000),
) -> ClusterUMAPResponse:
    """
    ...
    try:
        ...
        logger.error(f"Error retrieving UMAP projections for user {current_user.id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve UMAP projections",
        )
