...
async def get_ingest_events_endpoint(
    ...
    try:
        ...
        event_responses = [
            IngestEventResponse.model_validate(event) for event in events
        ]
        ...
        logger.error(
            f"Error retrieving ingest events for user {current_user.id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
...
@router.get(
    "/{event_id}", status_code=status.HTTP_200_OK, response_model=IngestEventResponse
)
async def get_ingest_event_endpoint(
    ...
    try:
        ...
        logger.error(
            f"Error retrieving ingest event {event_id} for user {current_user.id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
