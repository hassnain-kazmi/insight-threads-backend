...
async def get_cluster_umap_projections(
    ...
    cluster_query = select(Cluster).where(Cluster.id == cluster_id, Cluster.user_id == user_id)
    ...
    query = (
        ...
        .outerjoin(
            UMAPProjection,
            (UMAPProjection.document_id == Document.id) & (UMAPProjection.model_name == model_name)
        )
        .where(
    ...
    logger.info(f"Retrieved {len(projections)} UMAP projections for cluster {cluster_id}")
...
async def get_user_umap_projections(
    ...
    projections = [
        (document, projection, cluster_id)
        for document, projection, cluster_id in rows
    ]
