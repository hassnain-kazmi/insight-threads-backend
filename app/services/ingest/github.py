    ...
    try:
        ...
        while len(commits) < limit:
            ...
            response = requests.get(
                url, headers=_get_github_headers(), params=params, timeout=30.0
            )
...
def _fetch_pull_requests(
    ...
    try:
        ...
        logger.error(
            f"Failed to fetch pull requests from {owner}/{repo}: {e}", exc_info=True
        )
        return []
...
def _fetch_releases(
    ...
    try:
        ...
        logger.error(
            f"Failed to fetch releases from {owner}/{repo}: {e}", exc_info=True
        )
        return []
    ...
    try:
        for item_type, item_data in all_items:
            try:
                ...
                existing = check_duplicate(db, dedupe_key, user_id)
                if existing:
                    duplicates += 1
                ...
                item_id = item_data.get(
                    "id", item_data.get("sha", item_data.get("number", "unknown"))
                )
                logger.error(
                    f"Error processing {item_type} {item_id}: {e}", exc_info=True
                )
        ...
        db.rollback()
        logger.error(
            f"Critical error during GitHub ingestion, rolling back transaction: {e}",
            exc_info=True,
        )
        raise
