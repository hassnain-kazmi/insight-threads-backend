...
async def get_documents(
    ...
    if cluster_id:
        query = query.join(ClusterMember, Document.id == ClusterMember.document_id).where(
            ClusterMember.cluster_id == cluster_id
        )
        count_query = count_query.join(ClusterMember, Document.id == ClusterMember.document_id).where(
            ClusterMember.cluster_id == cluster_id
        )
    
    if source_type:
        query = query.join(IngestEvent, Document.ingest_event_id == IngestEvent.id).where(
            IngestEvent.source == source_type.lower()
        )
        count_query = count_query.join(IngestEvent, Document.ingest_event_id == IngestEvent.id).where(
            IngestEvent.source == source_type.lower()
        )
    
    if sentiment_min is not None or sentiment_max is not None:
        query = query.join(DocumentSentiment, Document.id == DocumentSentiment.document_id)
        count_query = count_query.join(DocumentSentiment, Document.id == DocumentSentiment.document_id)
        
        if sentiment_min is not None:
            query = query.where(DocumentSentiment.combined_score >= sentiment_min)
            count_query = count_query.where(DocumentSentiment.combined_score >= sentiment_min)
        
        if sentiment_max is not None:
            query = query.where(DocumentSentiment.combined_score <= sentiment_max)
            count_query = count_query.where(DocumentSentiment.combined_score <= sentiment_max)
