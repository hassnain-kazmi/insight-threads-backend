...
class ClusterResponse(BaseModel):
    ...
    trending_score: float | None = Field(
        None, description="Trending score (0.0 to 1.0)"
    )
...
class ClustersListResponse(BaseModel):
    ...
    clusters: list[ClusterResponse] = Field(
        ..., description="List of clusters ordered by trending score"
    )
...
class TimeseriesSummaryResponse(BaseModel):
    ...
    avg_sentiment: float | None = Field(
        None, description="Average sentiment for this date"
    )
    ...
    created_at: datetime = Field(
        ..., description="Timeseries summary creation timestamp"
    )
...
class ClusterDetailResponse(BaseModel):
    ...
    keywords: list[KeywordResponse] = Field(
        default_factory=list, description="Keywords associated with cluster"
    )
    timeseries: list[TimeseriesSummaryResponse] = Field(
        default_factory=list, description="Timeseries summaries for cluster"
    )
    insights: list[InsightResponse] = Field(
        default_factory=list, description="AI-generated insights for cluster"
    )
